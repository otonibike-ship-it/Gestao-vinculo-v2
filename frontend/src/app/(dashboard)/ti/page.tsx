...
import { Search, Monitor, Store, Repeat, Link2, FileEdit, RotateCcw, Ban } from 'lucide-react'
...
export default function TIPage() {
  ...
  const { data: cartas, isLoading: isLoadingCartas } = useQuery({
    queryKey: ['cartas-correcao', 'aguardando_ti'],
    queryFn: () => cartaCorrecaoService.listar('aguardando_ti'),
  })

  const { data: estornos, isLoading: isLoadingEstornos } = useQuery({
    queryKey: ['solicitacoes-estorno', 'aguardando_ti'],
    queryFn: () => solicitacaoEstornoService.listar('aguardando_ti'),
  })

  const { data: cancelamentos, isLoading: isLoadingCancelamentos } = useQuery({
    queryKey: ['cancelamentos-venda', 'aguardando_ti'],
    queryFn: () => cancelamentoVendaService.listar('aguardando_ti'),
  })
          ...
          <div className="max-h-[280px] overflow-y-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {itens.map((item) => (
                  <tr key={item.id} onClick={() => onSelect(item)} className="hover:bg-brand-forest/5 transition-colors cursor-pointer">
                    {renderRow(item)}
                  </tr>
                ))}
              </tbody>
    ...
    <div className="space-y-4">
      <div className="bg-brand-forest/10 border border-brand-forest/20 rounded-xl px-5 py-3">
        <p className="text-sm text-brand-forest">
          Solicitações aguardando execução de TI. Clique em um item para aprovar ou reprovar.
        </p>
      </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="max-h-[280px] overflow-y-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {filtrados.map((v) => (
                  <tr key={v.id} onClick={() => setSelecionado(v)} className="hover:bg-brand-forest/5 transition-colors cursor-pointer">
                ...
                <tbody className="divide-y divide-slate-50">
                  {franquiasFiltradas.map((f) => (
                    <tr key={f.id} className="hover:bg-brand-mist/60 transition-colors">
                      <td className="px-5 py-3 font-medium text-slate-800">{f.nome_fantasia || f.razao_social}</td>
                      <td className="px-5 py-3 text-slate-500 text-xs font-mono">{f.cnpj || '—'}</td>
      ...
      {selecionado && <VinculoModal vinculo={selecionado} onClose={() => setSelecionado(null)} modo="ti" />}
      {selecionadoTroca && <TrocaPedidoModal troca={selecionadoTroca} onClose={() => setSelecionadoTroca(null)} modo="ti" />}
      {selecionadoLink && <LinkPagamentoModal link={selecionadoLink} onClose={() => setSelecionadoLink(null)} modo="ti" />}
      {selecionadoCarta && <CartaCorrecaoModal carta={selecionadoCarta} onClose={() => setSelecionadoCarta(null)} modo="ti" />}
      {selecionadoEstorno && <SolicitacaoEstornoModal estorno={selecionadoEstorno} onClose={() => setSelecionadoEstorno(null)} modo="ti" />}
      {selecionadoCancelamento && <CancelamentoVendaModal cancelamento={selecionadoCancelamento} onClose={() => setSelecionadoCancelamento(null)} modo="ti" />}
    </div>
  )
}
