...
import { Search, Receipt, Link2, FileEdit, RotateCcw, Ban } from 'lucide-react'
...
export default function FaturamentoPage() {
  ...
  const { data: links, isLoading: isLoadingLinks } = useQuery({
    queryKey: ['links-pagamento', 'aguardando_faturamento'],
    queryFn: () => linkPagamentoService.listar('aguardando_faturamento'),
  })

  const { data: cartas, isLoading: isLoadingCartas } = useQuery({
    queryKey: ['cartas-correcao', 'aguardando_faturamento'],
    queryFn: () => cartaCorrecaoService.listar('aguardando_faturamento'),
  })

  const { data: estornos, isLoading: isLoadingEstornos } = useQuery({
    queryKey: ['solicitacoes-estorno', 'aguardando_faturamento'],
    queryFn: () => solicitacaoEstornoService.listar('aguardando_faturamento'),
  })

  const { data: cancelamentos, isLoading: isLoadingCancelamentos } = useQuery({
    queryKey: ['cancelamentos-venda', 'aguardando_faturamento'],
    queryFn: () => cancelamentoVendaService.listar('aguardando_faturamento'),
  })
          ...
          <div className="max-h-[280px] overflow-y-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {itens.map((item) => (
                  <tr key={item.id} onClick={() => onSelect(item)} className="hover:bg-brand-teal/10 transition-colors cursor-pointer">
                    {renderRow(item)}
                  </tr>
                ))}
              </tbody>
    ...
    <div className="space-y-4">
      <div className="bg-brand-teal/15 border border-brand-teal/30 rounded-xl px-5 py-3">
        <p className="text-sm text-brand-pine">
          Solicitações aguardando sua análise. Clique em um item para aprovar ou reprovar.
        </p>
      </div>
      ...
      {selecionado && <TrocaPedidoModal troca={selecionado} onClose={() => setSelecionado(null)} modo="faturamento" />}
      {selecionadoLink && <LinkPagamentoModal link={selecionadoLink} onClose={() => setSelecionadoLink(null)} modo="faturamento" />}
      {selecionadoCarta && <CartaCorrecaoModal carta={selecionadoCarta} onClose={() => setSelecionadoCarta(null)} modo="faturamento" />}
      {selecionadoEstorno && <SolicitacaoEstornoModal estorno={selecionadoEstorno} onClose={() => setSelecionadoEstorno(null)} modo="faturamento" />}
      {selecionadoCancelamento && <CancelamentoVendaModal cancelamento={selecionadoCancelamento} onClose={() => setSelecionadoCancelamento(null)} modo="faturamento" />}
    </div>
  )
}
