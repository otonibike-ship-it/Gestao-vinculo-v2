...
**Carta de Correção dropped Financeiro from its flow on 2026-09-25** — it's effectively a 1-area form now (Faturamento only). `aprovar_carta`'s `aguardando_faturamento` branch now sets status straight to `fechado` instead of `aguardando_financeiro`; the `aguardando_financeiro` branch (and the rest of `_AREA_STATUS`/`_AREA_EMAIL_CONFIG`) was deliberately left in place, not deleted, purely so any carta that was already parked there when this shipped can still be closed out by a financeiro user — no new carta will ever reach it going forward. `aguardando_financeiro` joins `aguardando_ti` as a defined-but-unreachable status for this form (same "known cosmetic gap" pattern noted below for Financeiro's/TI's dashboard queues). The modal's `FluxoStepper` steps dropped the "Financeiro" node (now Franquia → Faturamento → Carta Gerada); `aguardando_financeiro` still maps into the `faturamento` step index so a leftover legacy card doesn't render a broken index.

Every endpoint follows the same shape: an `_AREA_STATUS` dict (area name → status enum value), `_AREA_EMAIL_CONFIG` (area name → `configuracoes` key for its notification email), `_area_atual()` (reverse lookup from the record's current status), and `_registrar_nota()` (appends `{area, texto, tipo: "aprovacao"|"reprovacao", data}` to the `historico_observacoes` JSON column — every form got this column added in migration `0016`). For 3+-area forms, `aprovar` accepts `destino: Optional[str]` (an area name, or `"concluir"` only from the final area); every form's `reprovar` accepts `destino: Optional[str]` (an area name, or `"franquia"`, which is also the default when omitted). Vinculo is the one 3+-area exception with extra logic: its `necessario_validacao` flag still picks the default next step (financeiro vs ti) when Comercial approves without an explicit destino, preserving the pre-existing skip-financeiro behavior. **Since 2026-09-21 that flag is also forced to `true` whenever the record enters `validacao_financeiro` by any path** (the picker's explicit `destino`, or a `reprovar` sent to financeiro) — the UI always sends an explicit `destino`, so the default branch that used to set it never ran and pedidos routed through Financeiro kept `false`, hiding the Financeiro step in `vinculo-modal.tsx`'s stepper and showing "Não" in the "Valid. Financeiro" column. Migration `0020` backfilled old rows (status, `observacoes_financeiro`, or a `financeiro` entry in `historico_observacoes`).
...
**That 2026-09-08 rollout had a bug in 3 of the 6 `aprovar_*` handlers, fixed 2026-09-25**: Link de Pagamento, Carta de Correção and Solicitação de Estorno merge `payload.anexos` *inline per status branch* rather than once after the if/elif (unlike Vinculo/Troca de Pedido/Cancelamento de Venda, which already did it unconditionally after the branch). In all three, only the *last* branch (the one that closes the record) had the merge — the *first* area's branch (Comercial for Estorno, Faturamento for Carta and Link) silently dropped whatever the first approver attached, even though the upload block above it was already unconditional and the file really did land in Cloudinary. Now all three merge `payload.anexos` once, after the if/elif, same as the other three forms — copy that shape (merge after the branch, not inside each one) in any new form.
